...
- **根治大包完整对比时的内存耗尽闪退（本轮闪退的最终根因）**：smali 对比此前会把每个类的完整文件内容全部读进内存（46 个 dex 约 50 万个 smali 文件、两棵树合计数 GB 至十几 GB），直接打爆 V8 堆；v1.4.2 中 worker 线程 OOM 默认会连带整个进程退出（表现为对比开始后无声闪退）。本版：
  1. smali 遍历只保留内容哈希/大小/行数，不再保留文件全文，大包内存占用从 GB 级降到百 MB 级
  2. worker 线程加 2GB 独立堆上限，超限时只终止对比线程并向界面返回明确错误，不再带走整个应用
  3. 对比线程的启动/成功/失败日志改为立即刷盘，下次日志可直接定位问题所处阶段
- **修复启动缓慢**：v1.4.2 启动时同步删除旧版遗留的反编译目录（每个数十万小文件约需 5 分钟，14 个目录阻塞启动超过 60 分钟、窗口全程无响应）。清理已改为完全后台异步执行，启动立即完成
- smali 列表中"修改"类的 +N/-N 行数不再在列表构建时计算（此前对每个修改类当场做全文 diff 再丢弃结果），改为点击查看时实时计算

### 说明

- 任务管理器中看到多个进程（含 --type=gpu-process 参数的进程）是 Electron/Chromium 的标准多进程架构，属正常现象

## [1.4.2] - 2026-09-25

### 修复

- **根治"完整反编译对比"阶段应用未响应/闪退**：真正根因是反编译完成后，`buildDiffReport` 在主进程内同步对比两棵反编译树（大 APK 如 46 个 dex、约数十万文件时耗时数分钟到数十分钟），主进程事件循环被完全阻塞——窗口冻结（Windows 弹出"未响应"）、心跳与日志全部停止，进程被结束任务后表现为静默闪退，且日志缓冲中最后几行丢失。本版将对比计算整体移入 worker_threads 独立线程执行：
  1. 对比期间主进程保持完全响应：窗口可拖动、进度与日志正常、不再出现"未响应"
  2. 对比阶段提示明确文案"正在对比反编译结果（大包可能需要数分钟，请耐心等待）"
  3. 对比线程加 30 分钟超时保护，超时自动终止并提示
- 启动清理扩展：自动清理旧版本（v1.4.0 及之前）遗留在 C 盘 %APPDATA% 下的反编译产物目录（可达数 GB），不再依赖用户手动删除

### 根因备注

- v1.4.1 的磁盘迁移与空间防护仍然有效（输出已落 D 盘、心跳含剩余空间监控）；v1.4.0 时的"C 盘写满"是并发发生的次要问题，并非本次闪退的主因

## [1.4.1] - 2026-09-24

### 修复

- **根治大 APK 完整反编译时应用闪退**：根因是反编译产物目录固定写在 C 盘 `%APPDATA%` 下，两个大 APK（128MB + 227MB、46 个 dex）完整反编译输出可达数 GB，把 C 盘剩余空间耗尽，主进程因磁盘满异常死亡，且磁盘满时日志也无法写入（表现为无任何报错的静默闪退）。本版四层修复：
  1. 反编译输出根目录迁移至数据盘 `D:\testcode\apk-diff-tool\apk-out`（D 盘不存在或剩余不足 10GB 时才降级回原位置）
  2. 反编译启动前磁盘空间预检：按 APK 体积 15 倍 + 1GB 余量估算所需空间，不足时直接提示并拒绝执行，不再开跑
  3. 反编译运行中每 30 秒监控输出盘剩余空间，低于 500MB 红线时主动终止反编译并给出明确提示（心跳日志同时记录剩余空间，便于以后诊断）
  4. 应用启动时自动清理上次运行遗留的反编译孤儿目录，运行中会话目录超过 4 个按 FIFO 清理，防止单次运行内无限膨胀
- **uncaughtException 增加弹窗提示**：主进程内部错误不再无声退出，弹窗显示错误信息（日志写不出来时的最后防线）

## [1.4.0] - 2026-09-24

### 新增

- **单文件内容差异查看（hash 模式）**：快速对比结果中点击任意变更文件即可查看内容差异，无需完整反编译。主进程按需从两个 APK 提取该文件：二进制 XML（AndroidManifest.xml / res 下的 AXML）自动转换为可读文本后逐行对比；常见文本文件（json/html/js/css/properties 等）按 UTF-8 逐行对比（含二进制嗅探保护与 512KB 截断保护）；图片、so 库等二进制给出大小对比说明；DEX 提示走完整反编译并附跳转按钮
- **AI 对话面板**：工具栏新增对话入口（右侧抽屉），可针对当前对比结果多轮提问（如"总结主要变化""哪些改动最可疑""这个文件改了什么"）。对比上下文自动注入（对比统计 + 变更文件清单，最多 100 条）；正在查看的单文件差异会作为焦点上下文一并提供。AI 回复可携带操作按钮（查看某文件差异 / 开始完整反编译），实现对话即操作。复用已有的 OpenAI 兼容 API 配置
- **AI 推荐方案下一步操作**：hash 模式的 AI 推荐提示条新增"完整反编译对比"（跳过 AI 推荐直接执行全量 apktool 对比）与"AI 分析"（打开对话面板）按钮，补齐推荐之后无交互的断点

### 修复

- 修复 hash 模式文件列表点击行无任何反应的问题（原实现仅切换了从未渲染的展开状态）
